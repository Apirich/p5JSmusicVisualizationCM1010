	...
	this.draw = function() {
		...
		for (var i = 0; i < this.plotsDown; i++) {
			for (var j = 0; j < this.plotsAcross; j++) {
				...
				rect(x + x * j + w * j, y + y * i + h * i, w, h);
            

				//add on the ticks
				// ??
                var cterX = x + x * j + w * j + w / 2;
                var btomY = y + y * i + h * i + h;
                this.ticks(cterX, btomY, this.frequencyBins[currentBin]);

				var energy = fourier.getEnergy(this.frequencyBins[currentBin]);

				//add the needle
				// ??
                this.needle(energy, cterX, btomY);
